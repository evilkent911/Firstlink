...
public abstract class SectionCursorAdapter extends CursorAdapter {
	...
	private void calculateSectionHeaders() {
		...
		while (c.moveToNext()) {
			final String group = convertTime(c.getString(mGroupColumn));

			if (!group.equals(previous)) {
				sectionsIndexer.put(i + count, group);
				previous = group;

				DeviceUtility.log(TAG, "Group " + group + "at position: " + (i + count));
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if( viewType == TYPE_NORMAL) {
			...
			if (convertView == null) {
				DeviceUtility.log(TAG, "Creating new view for section");
				...
				DeviceUtility.log(TAG, "Reusing view for section");
