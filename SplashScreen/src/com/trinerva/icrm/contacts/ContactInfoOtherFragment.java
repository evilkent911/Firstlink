...
public class ContactInfoOtherFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		getData = (MyApplication) getActivity().getApplicationContext();
	...
	void onCreateMyDialog(int id) {
		...
		case MY_DATE_DIALOG_ID:
			...
			if (birthday.getText().toString().equals("")) {
				...
				dateDlg = new DatePickerDialog(getActivity(), new DatePickerDialog.OnDateSetListener() {
				...
				System.out.println("yyy " + Integer.parseInt(month.format(date)));
				dateDlg = new DatePickerDialog(getActivity(), new DatePickerDialog.OnDateSetListener() {
					public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
						Time chosenDate = new Time();
						chosenDate.set(dayOfMonth, monthOfYear, year);
	...
	private static String MobileNumber(String phoneNo) {
	...
	private OnClickListener saveContact = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (strLastName.length() == 0) {
				...
				GuiUtility.alert(getActivity(), "", getString(R.string.account_name_is_required), getString(R.string.ok));
				doLockField(false);
...
//				work_phone.requestFocus();
				...
				if (strMobile.length() == 0) {
					...
					strMobile = MobileNumber(strMobile);
	...
	private class SaveContact extends AsyncTask<ContactDetail, Void, Boolean> {
		...
		protected Boolean doInBackground(ContactDetail... params) {
			...
			contactDetail.setCompanyId(getData.getCompanyId());
		...
		protected void onPostExecute(Boolean result) {
			...
			if (result.booleanValue() == true) {
				...
				// Intent iContact = new Intent(getActivity(),
				// ContactList.class);
	...
	private class LoadContact extends AsyncTask<HashMap<String, String>, Void, ContactDetail> {
		...
		protected void onPostExecute(ContactDetail result) {
			...
			if (result.getOtherPhone() != null) {
				MobileNumber(result.getOtherPhone());
				other_phone.setText(formatPhoneNumber(result.getOtherPhone()));
			}

			if (result.getWorkFax() != null) {				
				work_fax.setText(formatPhoneNumber(result.getWorkFax()));
			}
	...
	private BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals("editPage")) {
				...
				String strCompanyName = intent.getStringExtra("strCompanyName");
				String strLastName = intent.getStringExtra("strLastName");
				String strFirstName = intent.getStringExtra("strFirstName");
				strPrefix = intent.getStringExtra("strPrefix");
				String strJobTitle = intent.getStringExtra("strJobTitle");
				String strMobile = intent.getStringExtra("strMobile");
				String strEmail = intent.getStringExtra("strEmail");
				String strWorkPhone = intent.getStringExtra("strWorkPhone");
				String strWorkFax = intent.getStringExtra("strWorkFax");
				String strStreet = intent.getStringExtra("strStreet");
				String strCity = intent.getStringExtra("strCity");
				String strState = intent.getStringExtra("strState");
				String strZip = intent.getStringExtra("strZip");
				String strCountry = intent.getStringExtra("strCountry");

				// String strDepartment =
				// intent.getStringExtra("strDepartment");
				// String strOtherPhone =
				// intent.getStringExtra("strOtherPhone");
				// String strHomePhone = intent.getStringExtra("strHomePhone");
...
//					work_phone.requestFocus();
					...
					if (bSetNameTo) {
						...
						Contact contact = new Contact(Constants.DBHANDLER);
						intent2.putExtra(Constants.NAME_TO_ID, "2-" +(contact.getAllDataCount().getCount()+1));
